...
public class BumperCarAI : MonoBehaviour
{
    ...
    void PickRandomCar()
    {
        var Cars = GameObject.FindGameObjectsWithTag("BumperCarAI");
        CustomDestination = Cars[Random.Range(0, Cars.Length)].transform;
    }
    ...
    {
        //if (collision.gameObject.tag == "BumperCarAI")
        //{
        //    RandomPath();
        //}
        MovementTorque = -MovementTorque;
    }
    ...
    public void RandomPath() // Creates a path to a random destination
    {
        ...
        else
        {
            ...
            Vector3 randomPostion = Random.insideUnitSphere * 20;
            //randomPostion;
        ...
        {
            if (NavMesh.SamplePosition(destination, out NavMeshHit hit, 20, NavMesh.AllAreas) &&
    ...
    private void ApplyBrakes() // Apply brake torque 
    {
        ...
        front.brakeTorque = 5000;
        left.brakeTorque = 5000;
        right.brakeTorque = 5000;
    }


    private void UpdateWheels() // Updates the wheel's postion and rotation
    {
        //ApplyRotationAndPostion(front, wheelFL);
        //ApplyRotationAndPostion(left, wheelBL);
        //ApplyRotationAndPostion(right, wheelBR);
    }
    ...
    {
        // If position we need to get to is behind the car, then we need to reverse

        /*if (Vector3.Distance(transform.position, PostionToFollow) > 0.5f)
        {
            if (Vector3.Angle(transform.forward, PostionToFollow - transform.position) > 90)
            {
                MovementTorque = -1;
            }
            else
        ...
        if (allowMovement == true)
        {
            ...
            int SpeedOfWheels = (int)((front.rpm + left.rpm + right.rpm) / 3);

            if (SpeedOfWheels < LocalMaxSpeed)
            {
                right.motorTorque = acceleration * MovementTorque;
                left.motorTorque = acceleration * MovementTorque;
                front.motorTorque = acceleration * MovementTorque;
            }
            else if (SpeedOfWheels < LocalMaxSpeed + (LocalMaxSpeed * 1 / 3))
            {
                right.motorTorque = 0;
                left.motorTorque = 0;
                front.motorTorque = 0;
